...
export const db = {
	...
	rules: {
		...
		create: (userId: number, repository: string, actions: string) =>
			getDb()
				.prepare("INSERT INTO access_rules (user_id, repository, actions) VALUES (?, ?, ?)")
				.run(userId, repository, actions),
    ...
    `,
		...
		insert: (e: Omit<Event, "id">) =>
			getDb()
				.prepare(
					"INSERT INTO events (action, repository, tag, digest, actor, ip, size, timestamp, raw) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
				)
	...
	webhooks: {
		findAll: () =>
			getDb().prepare("SELECT * FROM webhook_targets ORDER BY created_at DESC").all() as WebhookTarget[],
		findById: (id: number) =>
			getDb().prepare("SELECT * FROM webhook_targets WHERE id = ?").get(id) as WebhookTarget | undefined,
		...
		create: (w: Omit<WebhookTarget, "id" | "created_at" | "last_triggered" | "last_status">) =>
			getDb()
				.prepare(
					"INSERT INTO webhook_targets (name, repository_pattern, url, secret, events, active) VALUES (?, ?, ?, ?, ?, ?)",
				)
		...
		update: (id: number, fields: Partial<WebhookTarget>) => {
			...
			const sets = Object.keys(safe)
				...
				.join(", ");
			return getDb()
				.prepare(`UPDATE webhook_targets SET ${sets} WHERE id = ?`)
				.run(...Object.values(safe), id);
		},
	...
	cleanup: {
		...
		findById: (id: number) =>
			getDb().prepare("SELECT * FROM cleanup_rules WHERE id = ?").get(id) as CleanupRule | undefined,
		...
		create: (r: Omit<CleanupRule, "id" | "created_at" | "last_run" | "last_deleted">) =>
			getDb()
				.prepare(
					"INSERT INTO cleanup_rules (name, repository_pattern, keep_last_n, max_age_days, delete_untagged, active) VALUES (?, ?, ?, ?, ?, ?)",
				)
		...
		update: (id: number, fields: Partial<CleanupRule>) => {
			...
			const sets = Object.keys(safe)
				...
				.join(", ");
			return getDb()
				.prepare(`UPDATE cleanup_rules SET ${sets} WHERE id = ?`)
				.run(...Object.values(safe), id);
		},
	...
	settings: {
		get: (key: string) =>
			(getDb().prepare("SELECT value FROM settings WHERE key = ?").get(key) as { value: string } | undefined)
				?.value,
	...
	tokens: {
		findByUser: (userId: number) =>
			getDb()
				.prepare(
					"SELECT id, user_id, name, scope, expires_at, last_used, created_at FROM tokens WHERE user_id = ? ORDER BY created_at DESC",
				)
				.all(userId) as Omit<Token, "token_hash">[],
		...
		create: (userId: number, name: string, hash: string, scope: string, expiresAt?: string) =>
			getDb()
				.prepare("INSERT INTO tokens (user_id, name, token_hash, scope, expires_at) VALUES (?, ?, ?, ?, ?)")
				.run(userId, name, hash, scope, expiresAt ?? null),
		touch: (id: number) =>
			getDb().prepare("UPDATE tokens SET last_used = ? WHERE id = ?").run(new Date().toISOString(), id),
		delete: (id: number, userId?: number) => {
			if (userId !== undefined)
				return getDb().prepare("DELETE FROM tokens WHERE id = ? AND user_id = ?").run(id, userId);
	...
	groups: {
		...
		update: (id: number, fields: Partial<Group>) => {
			...
			const sets = Object.keys(safe)
				...
				.join(", ");
			return getDb()
				.prepare(`UPDATE groups SET ${sets} WHERE id = ?`)
				.run(...Object.values(safe), id);
		},
		...
		members: (groupId: number) =>
			getDb()
				.prepare(
					"SELECT u.id, u.username, u.role FROM users u JOIN group_members gm ON gm.user_id = u.id WHERE gm.group_id = ?",
				)
				.all(groupId) as Pick<User, "id" | "username" | "role">[],
		userGroups: (userId: number) =>
			getDb()
				.prepare("SELECT g.* FROM groups g JOIN group_members gm ON gm.group_id = g.id WHERE gm.user_id = ?")
				.all(userId) as Group[],
	...
	scans: {
		findAll: () =>
			getDb()
				.prepare(
					"SELECT id, repository, tag, digest, scanned_at, status, critical, high, medium, low FROM scan_results ORDER BY scanned_at DESC",
				)
				.all() as Omit<ScanResult, "raw_json">[],
		...
		upsert: (r: Omit<ScanResult, "id">) =>
			getDb()
				.prepare(
					`INSERT INTO scan_results (repository, tag, digest, scanned_at, status, critical, high, medium, low, raw_json)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
		...
		insert: (r: Omit<ScanResult, "id">) =>
			getDb()
				.prepare(
					"INSERT INTO scan_results (repository, tag, digest, scanned_at, status, critical, high, medium, low, raw_json) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
				)
				.run(
					r.repository,
					r.tag,
	...
	audit: {
		...
		search: (q: string, limit = 100) =>
			getDb()
				.prepare(
					"SELECT * FROM audit_log WHERE actor LIKE ? OR action LIKE ? OR target_type LIKE ? ORDER BY timestamp DESC LIMIT ?",
				)
	...
	sso: {
		...
		findActive: () =>
			getDb().prepare("SELECT * FROM sso_providers WHERE active = 1 ORDER BY name").all() as SsoProvider[],
		findById: (id: number) =>
			getDb().prepare("SELECT * FROM sso_providers WHERE id = ?").get(id) as SsoProvider | undefined,
		...
		update: (id: number, fields: Partial<SsoProvider>) => {
			...
			const sets = Object.keys(safe)
				...
				.join(", ");
			return getDb()
				.prepare(`UPDATE sso_providers SET ${sets} WHERE id = ?`)
				.run(...Object.values(safe), id);
		},
		delete: (id: number) => getDb().prepare("DELETE FROM sso_providers WHERE id = ?").run(id),
		saveState: (state: string, providerId: number) =>
			getDb()
				.prepare("INSERT INTO oauth_states (state, provider_id, created_at) VALUES (?, ?, ?)")
				.run(state, providerId, new Date().toISOString()),
		consumeState: (state: string) => {
			const row = getDb()
				.prepare('SELECT * FROM oauth_states WHERE state = ? AND created_at > datetime("now", "-5 minutes")')
				.get(state) as { provider_id: number } | undefined;
	...
	storage: {
		latest: () =>
			getDb()
				.prepare(
					"SELECT repository, registry_type, size_bytes, blob_count, MAX(snapshotted_at) as snapshotted_at FROM storage_snapshots GROUP BY repository, registry_type ORDER BY size_bytes DESC",
				)
				.all() as StorageSnapshot[],
		upsert: (repository: string, type: string, sizeBytes: number, blobCount: number) =>
			getDb()
				.prepare(
					"INSERT INTO storage_snapshots (repository, registry_type, size_bytes, blob_count) VALUES (?, ?, ?, ?)",
				)
