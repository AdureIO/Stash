...
import { db } from './db'
import { listRepositories, listTags, getManifest, getImageConfig, deleteManifest } from './registry'
...
export async function runCleanup(ruleId?: number): Promise<CleanupResult> {
  const rules = ruleId
    ? [db.cleanup.findById(ruleId)].filter(Boolean)
    : db.cleanup.findActive()
  ...
  for (const rule of rules) {
    ...
    for (const repo of matchingRepos) {
      ...
      if (rule.keep_last_n != null) tagDetails.slice(rule.keep_last_n).forEach(t => toDelete.add(t.digest))
      if (rule.max_age_days != null) {
        const cutoff = Date.now() - rule.max_age_days * 86400000
        tagDetails.forEach(t => { if (t.created && new Date(t.created).getTime() < cutoff) toDelete.add(t.digest) })
      }
