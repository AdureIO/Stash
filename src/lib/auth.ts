...
import { isPanelAdminRole, isSuperAdminRole } from "./roles";
...
/** Global super-admin only (settings, SSO, webhooks, audit, …). */
export async function requireSuperAdmin(): Promise<Session> {
	const session = await requireSession();
	if (!isSuperAdminRole(session.role)) throw new Error("Forbidden");
	return session;
}

/** Super-admin or space admin (users/groups within scope). */
export async function requirePanelAdmin(): Promise<Session> {
	const session = await requireSession();
	if (!isPanelAdminRole(session.role)) throw new Error("Forbidden");
	return session;
}

/** @deprecated Use requireSuperAdmin or requirePanelAdmin */
