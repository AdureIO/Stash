...
import { login, SESSION_COOKIE, createSession } from "@/lib/auth";
...
export async function POST(req: NextRequest) {
	try {
		...
		const ip = req.headers.get("x-real-ip")?.trim() || (xff ? xff.split(",").at(-1)!.trim() : null) || "unknown";
      ...
      res.cookies.set(SESSION_COOKIE, partialToken, sessionCookieOptions(300, req.headers.get('x-forwarded-proto')));
			return res;
		}

		logAction(username, "auth.login", "user", user?.id, undefined, ip);
		const res = NextResponse.json({ ok: true });
    res.cookies.set(SESSION_COOKIE, token, sessionCookieOptions(SESSION_DURATION, req.headers.get('x-forwarded-proto')));
		return res;
	} catch (err) {
