...
export function CleanupList({ rules: initial }: Props) {
	...
	async function handleRun(rule: CleanupRule) {
		...
		const { data } = await apiFetch<{ deleted: number; gc?: { ok: boolean; output: string; skipped?: boolean } }>(
			`/api/cleanup/${rule.id}/run`,
			{ method: "POST" },
		);
		setRunResult({ id: rule.id, deleted: data?.deleted ?? 0, gc: data?.gc });
		setRunningId(null);
		await refresh();
	}
	...
	return (
		<>
			...
			{runResult && (
				<div
					className={`mb-4 px-4 py-3 rounded-lg border text-sm flex items-center justify-between ${runResult.gc && !runResult.gc.ok && !runResult.gc.skipped ? "bg-amber-50 border-amber-200 text-amber-900" : "bg-green-50 border-green-200 text-green-800"}`}
				>
					<span>
						Cleanup complete — <strong>{runResult.deleted}</strong> tag{runResult.deleted !== 1 ? "s" : ""}{" "}
						deleted
						{runResult.gc && !runResult.gc.skipped && (
							<>, garbage collection {runResult.gc.ok ? "completed" : "failed"}</>
						)}
					</span>
					<button onClick={() => setRunResult(null)} className="opacity-70 hover:opacity-100">
						✕
					</button>
				</div>
			)}
					...
					<Tbody>
						{rules.map((r) => (
							<Tr key={r.id}>
								...
								<Td>
									<code className="text-xs bg-zinc-50 border border-zinc-100 px-1.5 py-0.5 rounded">
										{r.repository_pattern}
									</code>
								</Td>
								<Td>{r.keep_last_n ?? "—"}</Td>
			...
			<Dialog open={addOpen} onClose={() => setAddOpen(false)} title="Add cleanup rule">
				<form onSubmit={handleAdd} className="space-y-4">
					...
					<Input
						...
						placeholder="* (all repos) or myapp/*"
					/>
			...
			<Dialog open={!!deleteItem} onClose={() => setDeleteItem(null)} title="Delete rule">
				<p className="text-sm text-zinc-600 mb-5">
					Delete rule <span className="font-medium">{deleteItem?.name}</span>?
				</p>
				<div className="flex gap-2 justify-end">
					<Button variant="secondary" size="sm" onClick={() => setDeleteItem(null)}>
						Cancel
					</Button>
