...
export default function TotpPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    else { setError(err || 'Invalid code'); setCode(''); inputRef.current?.focus() }
    setLoading(false)
  }
    ...
    <div className="min-h-screen bg-zinc-900 flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        ...
        <form onSubmit={handleSubmit} className="bg-zinc-800 rounded-xl p-6 space-y-4 border border-zinc-700">
          ...
          <button type="button" onClick={() => { document.cookie = 'ra_session=; Max-Age=0'; router.push('/login') }} className="w-full text-center text-sm text-zinc-500 hover:text-zinc-300">
            Back to login
          </button>
        </form>
      </div>
