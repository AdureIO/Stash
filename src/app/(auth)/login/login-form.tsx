...
interface SsoProvider { id: number; name: string; type: string }
interface Props { ssoProviders: SsoProvider[] }

const SSO_ICONS: Record<string, string> = {
  google: 'G', github: '⌬', gitlab: '⬡', oidc: '◎',
}
    ...
    <div className="min-h-screen bg-zinc-900 flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        ...
        <form onSubmit={handleSubmit} className="bg-zinc-800 rounded-xl p-6 space-y-4 border border-zinc-700">
          <Input id="username" label="Username" labelClassName="text-zinc-300" type="text" value={username}
          ...
          <Input id="password" label="Password" labelClassName="text-zinc-300" type="password" value={password}
            onChange={e => setPassword(e.target.value)} placeholder="••••••••" autoComplete="current-password" required
            className="bg-zinc-700 border-zinc-600 text-white placeholder:text-zinc-500 focus:border-blue-500" />

          {error && <p className="text-sm text-red-400 flex items-center gap-1.5"><AlertCircle size={13} />{error}</p>}
        ...
        {ssoProviders.length > 0 && (
          <div className="mt-4 space-y-2">
            <div className="flex items-center gap-3 text-zinc-600 text-xs"><div className="flex-1 h-px bg-zinc-700" />or continue with<div className="flex-1 h-px bg-zinc-700" /></div>
            {ssoProviders.map(p => (
              <a key={p.id} href={`/api/auth/sso/${p.id}`}
                ...
                <span className="w-5 h-5 bg-zinc-700 rounded text-xs flex items-center justify-center font-bold">{SSO_ICONS[p.type] || p.type[0].toUpperCase()}</span>
                {p.name}
              </a>
            ))}
          </div>
